...
tf.flags.DEFINE_integer("max_sequence_len", 100, "句子最大长度，问句和答案都一样")
...
train_data = data_helpers.load_data(FLAGS.traindata_file) 
test_data = data_helpers.load_data(FLAGS.testdata_file)
    ...
    with sess.as_default():
        ...
        for batch in train_batches:
            ...
            if step % FLAGS.evaluate_every == 0:
                logging.info("\nEvalution:")
                test_encoder_inputs, test_encoder_inputs_actual_lengths, test_decoder_outputs, test_decoder_outputs_actual_lengths = zip(*test_batch)
                feed_dict = {
                ...
                _, test_step, test_loss = sess.run([model.train_op, model.global_step, model.loss], feed_dict=feed_dict)
